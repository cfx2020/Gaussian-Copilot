...
- 分区式后续输入生成：
  ...
  - `继续优化`
  - `IRC 路径验证`
- `TS 前后中间体` 直接集成在 TS 区块中，可预览前/后位移并一次生成两个 `OPT+FREQ` 输入
- 支持 `从当前帧继续优化`：
  - 默认输出为原文件名后加 `1`（如 `foo1.gjf`）
...
1. 用模板生成/编辑 `.gjf`
2. 单个或批量提交作业
3. 在作业栏查看时间线、调度状态与终止结果
4. 单击作业快速打开结果
5. 在可视化面板中检查几何、频率、能量曲线与终止状态
6. 在 `Next` 面板里直接生成 TS / Sol / 继续优化 / IRC 后续输入
