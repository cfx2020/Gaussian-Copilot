...
### Fixed
- 修复作业看板 SSH 挂机恢复后自动刷新重入导致的持续刷新闪烁问题。
- 修复终端提交作业自动发现不稳定的问题：调度器查询增加超时与并发限制，并记录 `qstat -f` 返回的工作目录线索。
- 修复清理/删除作业后刷新又被自动发现加回来的问题。
- 移除同名 `.log/.out/.gjf` 的手动选择弹窗；现在只在能够唯一定位时打开文件，避免自动刷新或选中作业时反复弹窗。
...
### Fixed
- 修复 Sol 基组升级对 `6-31g(d)` / `6-31g(d,p)` 的正则匹配错误：现在无论大小写、无论普通基组还是 `genecp` 尾部，都能正确升级为 `6-311++G(d,p)`。

## [0.6.1] - 2026-04-14

### Fixed
- 修复从任一 Sol 按钮生成输入文件时命名不一致的问题：现在统一输出为 `<name>_sol.gjf`。
...
- 输出终止状态解析增强：支持区分正常结束、报错结束与运行中，并将结果反馈到可视化面板与作业视图。
...
- Webview 面板控件、工具栏状态提示与悬浮区交互细节整体打磨，减少状态混乱。
- 右侧信息面板 glass 风格与按钮对比度优化，提升深色背景下的可读性。

### Fixed
- 修复“从当前帧继续优化”错误重写任务类型的问题：现在优先继承原 `.gjf` 的 route，适用于 TS、fix、opt 等任务的中途续跑。
- 修复 Next 面板顶部冗余总标题，减少层级噪音并强化任务分区。
- 修复查看器工具栏与状态区在部分终止状态下信息不完整的问题。
